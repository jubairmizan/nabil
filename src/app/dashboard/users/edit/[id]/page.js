...
function EditUserPage() {
  ...
  // Fetch shifts when role is billing_counter
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        setFormData({
          ...
          assigned_section: userData.assigned_section || "ALL",
          shift_id: userData.shift_id || "",
          password: "", // Leave password blank for edit form
        });
      } catch (err) {
  ...
  const handleChange = (e) => {
    ...
    // If role changes from billing_counter, clear shift_id
  ...
  const validateForm = () => {
    ...
    // Password is optional for updates, but if provided, check length
    ...
    try {
      ...
      if (error.message !== "Validation failed") {
        Swal.fire({
          ...
          text: error.message || "There was a problem updating the user",
          confirmButtonColor: "#EF4444",
        });
      }
    } finally {
      ...
      <div className={styles.productsContainer}>
        <div className={styles.errorState}>
          ...
          <div className={styles.errorDetails}>
            <p>
              <strong>API URL:</strong> {getApiBaseUrl()}/api/users/{id}
            </p>
          </div>
          <Link href="/dashboard/users" className={styles.addButton}>
            Back to Users
          </Link>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className={styles.formGroup} style={{ marginBottom: "25px" }}>
            <label htmlFor="password" style={{ display: "block", marginBottom: "8px", fontWeight: "500" }}>
              Password <span style={{ color: "#6c757d", fontWeight: "400", fontSize: "0.85rem" }}>(Leave blank to keep current password)</span>
            </label>
            <input
              type="password"
              id="password"
