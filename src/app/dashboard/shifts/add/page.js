...
function AddShiftPage() {
  ...
  // Fetch existing shifts for time overlap validation
  useEffect(() => {
    const fetchShifts = async () => {
      try {
        ...
        const response = await fetch(`${getApiBaseUrl()}/api/shifts`, {
          headers: {
    ...
    <div className={styles.productsContainer}>
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          <h1>Add New Shift</h1>
          <p>Create a new shift for staff scheduling</p>
        </div>
        <Link
...
export default function ProtectedAddShiftPage() {
  return (
